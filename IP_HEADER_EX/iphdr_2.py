...
def unpack_Iphdr(buffer): # 언팩하여 표현
    unpacked = struct.unpack('!BBHHHBBH4s4s', buffer[:20])
    return unpacked

def getPacketSize(unpacked_ipheader): # 언팩한 헤더에서 패킷크기 추출하는 함수
    return unpacked_ipheader[2]

def getProtocolId(unpacked_ipheader): # 언팩한 헤더에서 프로토콜 종류 추출하는 함수
    return unpacked_ipheader[6]

def getIP(unpacked_ipheader): # 언팩한 헤더에서 ip주소 추출하는 함수
